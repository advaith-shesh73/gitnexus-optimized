...
import { executeQuery, closeLbug, withLbugDb, isLbugReady } from '../core/lbug/lbug-adapter.js';
...
const statusFromError = (err: any): number => {
  ...
  // During a blue-green swap the DB file may briefly not exist or be locked
  if (msg.includes('busy') || msg.includes('lock') || msg.includes('enoent') || msg.includes('no such file')) return 503;
  return 500;
};
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  // Health check — reports DB readiness and registered repo count.
  // During a blue-green swap the DB may be momentarily unavailable;
  // callers should retry on 503.
  app.get('/api/health', async (_req, res) => {
    try {
      const repos = await listRegisteredRepos();
      const dbReady = isLbugReady();
      const status = repos.length > 0 ? (dbReady ? 'ok' : 'reconnecting') : 'no_repos';
      const code = status === 'ok' ? 200 : status === 'reconnecting' ? 503 : 200;
