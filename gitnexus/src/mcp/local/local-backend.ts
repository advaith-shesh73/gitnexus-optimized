...
/**
 * Detect generated code files (protobuf, thrift, graphql, swagger, etc.).
 * These should rank below their source definitions in search results.
 */
function isGeneratedFilePath(fp: string): boolean {
  const lower = fp.toLowerCase();
  return (
    lower.endsWith('.pb.go') || lower.endsWith('.pb.cc') ||
    lower.endsWith('.pb.h') || lower.endsWith('.pb.swift') ||
    lower.endsWith('_pb2.py') || lower.endsWith('_pb2_grpc.py') ||
    lower.endsWith('.grpc.pb.go') || lower.endsWith('.grpc.pb.cc') ||
    lower.includes('/gen-') || lower.includes('/generated/') ||
    lower.endsWith('.generated.cs') || lower.endsWith('.g.cs') ||
    lower.endsWith('.designer.cs') ||
    lower.endsWith('.generated.ts') || lower.endsWith('.generated.js') ||
    lower.endsWith('_gen.go')
  );
}

/**
 * Detect C/C++/ObjC implementation files that may have a header counterpart.
 */
function isImplementationFilePath(fp: string): boolean {
  const ext = fp.slice(fp.lastIndexOf('.'));
  return (
    ext === '.cc' || ext === '.cpp' || ext === '.cxx' || ext === '.c++' ||
    ext === '.c' || ext === '.m' || ext === '.mm'
  );
}
...
interface ScopePrefix {
  prefix: string;
  confidence: number;
}

/**
 * Extract directory-like scope prefixes from task_context.
 *
 * Only extracts from path-like tokens containing "/" to avoid false
 * positives from bare English words ("store", "config", "service").
 * Returns prefixes with confidence scores:
 *   - Multi-segment path (minerva/lib):  1.0
 *   - Single-segment path (minerva/):    0.8
 */
function extractScopePrefixes(taskContext?: string): ScopePrefix[] {
  if (!taskContext) return [];
  const seen = new Set<string>();
  const prefixes: ScopePrefix[] = [];

  const pathTokens = taskContext.match(/[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_/.-]*/g);
  if (pathTokens) {
    for (const t of pathTokens) {
      const segments = t.split('/').filter(Boolean);
      if (segments.length >= 2 && !seen.has(segments[0])) {
        seen.add(segments[0]);
        prefixes.push({ prefix: segments[0], confidence: 1.0 });
      } else if (segments.length === 1 && segments[0].length > 1 && !seen.has(segments[0])) {
        seen.add(segments[0]);
        prefixes.push({ prefix: segments[0], confidence: 0.8 });
      }
    }
  }
  return prefixes;
...
export class LocalBackend {
  ...
  private async query(repo: RepoHandle, params: {
    ...
    // Subsystem scoping: extract directory prefixes from task_context to boost
    // results whose filePath shares a common subsystem directory.
    // Boost is adaptive: stronger when fewer prefixes match (targeted scope)
    // and weighted by prefix confidence (path-derived > bare word).
    const scopePrefixes = extractScopePrefixes(params.task_context);

    const topLevelDirs = new Set<string>();
    if (scopePrefixes.length > 0) {
      for (const [, item] of scoreMap) {
        const fp = item.data.filePath as string | undefined;
        if (fp) {
    ...
    const subsystemCount = Math.max(topLevelDirs.size, 1);

    const merged = Array.from(scoreMap.entries())
      .map(([key, item]) => {
        const fp = item.data.filePath as string | undefined;

        // Subsystem scope boost
        if (scopePrefixes.length > 0 && fp) {
          let bestConfidence = 0;
          for (const sp of scopePrefixes) {
            if (fp.startsWith(sp.prefix + '/') || fp.includes('/' + sp.prefix + '/')) {
              bestConfidence = Math.max(bestConfidence, sp.confidence);
            }
          }
          if (bestConfidence > 0) {
            const boost = 1 + subsystemCount * bestConfidence;
            item.score *= boost;
          }
        }

        // Generated-file demotion: protobuf/thrift/graphql generated code
        // should rank below their source definitions
        if (fp && isGeneratedFilePath(fp)) {
          item.score *= 0.5;
        }
    ...
    // Step 1b: Declaration-aware enrichment (batched) — collect all impl nodeIds,
    // fire a single DEFINED_BY query, then inject header counterparts.
    const implNodeIds: string[] = [];
    const implScoreByNodeId = new Map<string, number>();
    for (const [, item] of merged) {
      const fp = item.data.filePath as string | undefined;
      const nodeId = item.data.nodeId as string | undefined;
      if (!nodeId || !fp) continue;
      if (!isImplementationFilePath(fp)) continue;
      implNodeIds.push(nodeId);
      implScoreByNodeId.set(nodeId, item.score);
    }
    if (implNodeIds.length > 0) {
      try {
        const declRows = await executeParameterized(repo.id, `
          UNWIND $implIds AS iid
          MATCH (impl {id: iid})-[:CodeRelation {type: 'DEFINED_BY'}]->(decl)
          RETURN impl.id AS implId, decl.id AS id, decl.name AS name, labels(decl)[0] AS type,
                 decl.filePath AS filePath, decl.startLine AS startLine, decl.endLine AS endLine
        `, { implIds: implNodeIds });
        for (const dr of declRows) {
          const implId = dr.implId ?? dr[0];
          const declId = dr.id ?? dr[1];
          const declFp = dr.filePath ?? dr[4];
          if (declId && declFp && !scoreMap.has(declId)) {
            const implScore = implScoreByNodeId.get(implId) ?? 0;
            merged.push([declId, {
              score: implScore * 1.15,
              data: {
                nodeId: declId,
                name: dr.name ?? dr[2],
                type: dr.type ?? dr[3],
                filePath: declFp,
                startLine: dr.startLine ?? dr[5],
                endLine: dr.endLine ?? dr[6],
              },
            }]);
          }
        }
        merged.sort((a, b) => b[1].score - a[1].score);
      } catch (e) { logQueryError('query:declaration-lookup-batch', e); }
    }

    // Step 2: Batch-lookup process membership and community info for ALL nodeIds
    // instead of per-symbol sequential queries (eliminates N+1).
    const allNodeIds: string[] = [];
    const nodeIdToMergedIdx = new Map<string, number[]>();
    for (let i = 0; i < merged.length; i++) {
      const sym = merged[i][1].data;
      if (sym.nodeId) {
        allNodeIds.push(sym.nodeId);
        const indices = nodeIdToMergedIdx.get(sym.nodeId) ?? [];
        indices.push(i);
        nodeIdToMergedIdx.set(sym.nodeId, indices);
      }
    }

    // Batch process membership query
    const processByNodeId = new Map<string, any[]>();
    if (allNodeIds.length > 0) {
      try {
        const processRows = await executeParameterized(repo.id, `
          UNWIND $nodeIds AS nid
          MATCH (n {id: nid})-[r:CodeRelation {type: 'STEP_IN_PROCESS'}]->(p:Process)
          RETURN n.id AS nodeId, p.id AS pid, p.label AS label,
                 p.heuristicLabel AS heuristicLabel, p.processType AS processType,
                 p.stepCount AS stepCount, r.step AS step
        `, { nodeIds: allNodeIds });
        for (const row of processRows) {
          const nid = row.nodeId ?? row[0];
          const arr = processByNodeId.get(nid) ?? [];
          arr.push(row);
          processByNodeId.set(nid, arr);
        }
      } catch (e) { logQueryError('query:process-lookup-batch', e); }
    }

    // Batch community membership query
    const communityByNodeId = new Map<string, { cohesion: number; module?: string }>();
    if (allNodeIds.length > 0) {
      try {
        const cohesionRows = await executeParameterized(repo.id, `
          UNWIND $nodeIds AS nid
          MATCH (n {id: nid})-[:CodeRelation {type: 'MEMBER_OF'}]->(c:Community)
          RETURN n.id AS nodeId, c.cohesion AS cohesion, c.heuristicLabel AS module
        `, { nodeIds: allNodeIds });
        for (const row of cohesionRows) {
          const nid = row.nodeId ?? row[0];
          if (!communityByNodeId.has(nid)) {
            communityByNodeId.set(nid, {
              cohesion: (row.cohesion ?? row[1]) || 0,
              module: row.module ?? row[2],
            });
          }
        }
      } catch (e) { logQueryError('query:cluster-info-batch', e); }
    }

    // Batch content fetch (only if requested)
    const contentByNodeId = new Map<string, string>();
    if (includeContent && allNodeIds.length > 0) {
      try {
        ...
        for (const row of contentRows) {
          const nid = row.nodeId ?? row[0];
          const c = row.content ?? row[1];
          if (c) contentByNodeId.set(nid, c);
        }
      } catch (e) { logQueryError('query:content-fetch-batch', e); }
    }

    // Assemble: build process map and definitions from batch results
    ...
    for (const [, item] of merged) {
      ...
      const community = communityByNodeId.get(sym.nodeId);
      const cohesion = community?.cohesion ?? 0;
      const module = community?.module;
      const content = contentByNodeId.get(sym.nodeId);
      ...
      const processRows = processByNodeId.get(sym.nodeId) ?? [];
   ...
   * Returns symbol-level results directly from FTS (no per-result DB lookups).
   */
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<{ results: any[]; ftsUsed: boolean }> {
    ...
    try {
      const report = await searchFTSFromLbug(query, limit, repo.id);
      ...
      return { results, ftsUsed: report.tablesWithResults > 0 };
    } catch (err: any) {
