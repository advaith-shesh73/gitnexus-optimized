 ...
 * The MCP server handles concurrent tool calls (e.g. a user asks two
 * questions simultaneously). LadybugDB Connections are NOT thread-safe —
 * a single Connection segfaults if concurrent .query() calls hit it.
 * This adapter provides a checkout/return pool of Connection objects from
 * a shared Database so each concurrent query gets its own Connection.
 *
 * Additionally, LadybugDB writes to stdout during some operations (e.g.
 * community detection progress). Since the MCP server communicates via
 * JSON-RPC over stdout, we must silence stdout during queries to prevent
 * corruption. See silenceStdout()/restoreStdout().
 *
 * Multiple repoIds pointing to the same on-disk path share one native
 * Database object to avoid exhausting the buffer manager's mmap budget.
 *
...
export const executeQuery = async (repoId: string, cypher: string): Promise<any[]> => {
  ...
  try {
    ...
    if (elapsed > 500) {
      console.error(JSON.stringify({ level: 'warn', event: 'slow_query', elapsed_ms: elapsed, cypher: cypher.slice(0, 200), repoId }));
    }
    activeQueryCount--;
    restoreStdout();
    checkin(entry, conn);
