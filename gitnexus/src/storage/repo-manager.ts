...
export interface RepoMeta {
  ...
  lastCommit: string;
  indexedAt: string;
  /** True while an incremental reindex is in progress (delete→insert window).
   *  If the process crashes mid-reindex, this flag forces a full rebuild on
   *  the next run to recover from the inconsistent state. */
  dirty?: boolean;
  /** True when FTS indexes are stale (incremental skipped FTS rebuild).
   *  The MCP server will lazily rebuild before the first FTS query. */
  ftsStale?: boolean;
  stats?: {
    files?: number;
    nodes?: number;
...
/**
 * Save metadata to storage using atomic write-then-rename.
 * A crash during write leaves the old meta.json intact (or the tmp file
 * as garbage that loadMeta ignores). This prevents a half-written JSON
 * from corrupting the index state.
 */
...
/**
 * Write the global registry to disk using atomic write-then-rename.
 * Prevents concurrent `analyze` commands from corrupting the registry
