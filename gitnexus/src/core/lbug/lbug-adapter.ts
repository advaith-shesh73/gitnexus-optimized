...
/** Tracks the mtime of the connected DB file so the serve process can
 *  detect a blue-green swap and automatically reconnect. */
...
/**
 * Force the next query to reconnect, even if the dbPath hasn't changed.
 * Useful after a blue-green swap where the file at the same path is now a
...
const getFileMtimeMs = (filePath: string): number | null => {
  try {
...
const ensureLbugInitialized = async (dbPath: string) => {
  const currentMtime = getFileMtimeMs(dbPath);

  // Reuse existing connection only if the path matches AND the file on disk
  // hasn't been swapped (mtime unchanged).  After a blue-green rename the
  // mtime will differ, triggering a reconnect to the new database.
  if (conn && currentDbPath === dbPath && currentMtime !== null && currentMtime === lastKnownMtimeMs) {
    return { db, conn };
  }
  await doInitLbug(dbPath);
  lastKnownMtimeMs = getFileMtimeMs(dbPath);
  return { db, conn };
};
...
const doInitLbug = async (dbPath: string) => {
  ...
  // Create filePath indexes for incremental deletion performance
  for (const idxQuery of FILEPATH_INDEX_QUERIES) {
    try {
      await conn.query(idxQuery);
    } catch {
      // Index may already exist or table may not support it — safe to skip
    }
  }
...
/** Tables that have a `filePath` column and can be filtered for file-level deletion. */
const TABLES_WITH_FILEPATH = NODE_TABLES.filter(
  t => t !== 'Community' && t !== 'Process',
);

/**
 * Delete all nodes (and their relationships) belonging to the given file paths.
 * Uses DETACH DELETE so edges touching the deleted nodes are removed too.
 *
 * NOTE: This does NOT fix cross-file edges.  If file A imports from file B
 * and B is deleted, A's IMPORTS edge becomes a dangling reference.  Callers
 * that need full graph consistency should prefer a blue-green full rebuild.
 *
 * Intended for future incremental-update use; not wired into the main
 * analyze path yet.
 */
export const deleteNodesByFilePath = async (filePaths: string[]): Promise<number> => {
  if (!conn || filePaths.length === 0) return 0;
  ...
  for (const table of TABLES_WITH_FILEPATH) {
    const t = escapeTableName(table);
    for (const fp of filePaths) {
      try {
        await conn.query(
          `MATCH (n:${t}) WHERE n.filePath = ${escapeValue(fp)} DETACH DELETE n`,
        );
        // LadybugDB doesn't reliably return count after DETACH DELETE,
        // so we count best-effort via the number of paths processed.
        totalDeleted++;
      } catch {
        // Table may be empty or not exist — safe to skip
      }
    }
  }

  // Remove orphaned embeddings for ALL symbol types in the deleted files.
  // Embedding nodeIds are prefixed with the symbol type (e.g. "Function:src/auth.ts::login").
  const EMBEDDING_NODE_PREFIXES = ['File:', 'Function:', 'Class:', 'Method:', 'Interface:', 'CodeElement:'];
  for (const fp of filePaths) {
    for (const prefix of EMBEDDING_NODE_PREFIXES) {
      try {
        await conn.query(
          `MATCH (e:CodeEmbedding) WHERE e.nodeId STARTS WITH ${escapeValue(prefix + fp)} DETACH DELETE e`,
        );
      } catch { /* embedding table may not exist */ }
    }
  }
 ...
 * Must be called before a blue-green file rename to ensure the pending
 * database contains all committed data.  Safe to call even if there is
 * nothing to checkpoint.
 */
export const checkpointLbug = async (): Promise<void> => {
  if (!conn) return;
  try {
    await conn.query('CHECKPOINT');
  } catch {
    // Checkpoint can fail when other transactions are active; best-effort.
  }
};
...
/**
 * Load the algo extension (required before using graph algorithms like Louvain).
 * Safe to call multiple times — tracks loaded state via module-level algoLoaded.
 */
export const loadAlgoExtension = async (): Promise<void> => {
  if (algoLoaded) return;
  if (!conn) {
    throw new Error('LadybugDB not initialized. Call initLbug first.');
  }
  try {
    await conn.query('INSTALL algo');
    ...
    if (msg.includes('already loaded') || msg.includes('already installed') || msg.includes('already exists')) {
      ...
      console.error(
        '  Community detection unavailable (algo extension failed to load).\n' +
        '  Communities will use file-based fallback clustering.\n' +
...
import { FTS_INDEX_DEFS } from '../search/searchable-types.js';

/**
 * Drop all standard FTS indexes, then recreate them.
 * LadybugDB FTS indexes are non-incremental (no way to update individual
 * entries), so after node mutations we must rebuild from scratch.
 *
 * Crash-safe: tracks which indexes were dropped so that if we crash between
 * drop and create, the next call will retry the missing creations.
 */
export const rebuildFTSIndexes = async (): Promise<void> => {
  const dropped: typeof FTS_INDEX_DEFS[number][] = [];

  for (const def of FTS_INDEX_DEFS) {
    await dropFTSIndex(def.table, def.name);
    dropped.push(def);
  }

  for (const def of dropped) {
    try {
      await createFTSIndex(def.table, def.name, [...def.props]);
    } catch (err) {
      console.error(`FTS rebuild failed for ${def.table}/${def.name}, retrying once:`, err);
      await createFTSIndex(def.table, def.name, [...def.props]);
    }
  }
};
