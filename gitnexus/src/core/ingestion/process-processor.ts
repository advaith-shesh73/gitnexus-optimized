...
const buildCallsGraph = (graph: KnowledgeGraph): AdjacencyList => {
  ...
  const declaresMap = new Map<string, string>();
  const partialPeers = new Map<string, string[]>();
  ...
  for (const rel of graph.iterRelationships()) {
    ...
    } else if (rel.type === 'DECLARES') {
      declaresMap.set(rel.sourceId, rel.targetId);
    } else if (rel.type === 'PARTIAL_OF') {
      let peers = partialPeers.get(rel.sourceId);
      if (!peers) { peers = []; partialPeers.set(rel.sourceId, peers); }
      peers.push(rel.targetId);
    }
  }

  // DECLARES: header inherits implementation's callees
  for (const [headerId, implId] of declaresMap) {
    const implCallees = adj.get(implId);
    if (implCallees && implCallees.length > 0) {
      const existing = adj.get(headerId) || [];
      adj.set(headerId, [...existing, ...implCallees]);
    }
  }

  // PARTIAL_OF: each partial class inherits its peer partials' callees
  for (const [nodeId, peers] of partialPeers) {
    for (const peerId of peers) {
      const peerCallees = adj.get(peerId);
      if (peerCallees && peerCallees.length > 0) {
        const existing = adj.get(nodeId) || [];
        adj.set(nodeId, [...existing, ...peerCallees]);
      }
    }
  }
...
const buildReverseCallsGraph = (graph: KnowledgeGraph): AdjacencyList => {
  const adj = new Map<string, string[]>();
  const definedByMap = new Map<string, string>();
  const partialPeers = new Map<string, string[]>();
  ...
  for (const rel of graph.iterRelationships()) {
    ...
    } else if (rel.type === 'DEFINED_BY') {
      definedByMap.set(rel.sourceId, rel.targetId);
    } else if (rel.type === 'PARTIAL_OF') {
      let peers = partialPeers.get(rel.sourceId);
      if (!peers) { peers = []; partialPeers.set(rel.sourceId, peers); }
      peers.push(rel.targetId);
  ...
  // DEFINED_BY: impl inherits header's callers
  for (const [implId, headerId] of definedByMap) {
    const headerCallers = adj.get(headerId);
    if (headerCallers && headerCallers.length > 0) {
      const existing = adj.get(implId) || [];
      adj.set(implId, [...existing, ...headerCallers]);
    }
  }

  // PARTIAL_OF: each partial class inherits its peer partials' callers
  for (const [nodeId, peers] of partialPeers) {
    for (const peerId of peers) {
      const peerCallers = adj.get(peerId);
      if (peerCallers && peerCallers.length > 0) {
        const existing = adj.get(nodeId) || [];
        adj.set(nodeId, [...existing, ...peerCallers]);
      }
    }
  }
...
const deduplicateTraces = (traces: string[][]): string[][] => {
  ...
  for (const trace of sorted) {
    const isSubseq = unique.some(existing => isOrderedSubsequence(trace, existing));
    if (!isSubseq) {
      unique.push(trace);
    }
  }
...
/**
 * True if every element of `short` appears in `long` in the same relative
 * order (proper ordered subsequence check).  Uses O(|long|) scan per call,
 * which is acceptable because traces are short (typically <30 nodes).
 */
