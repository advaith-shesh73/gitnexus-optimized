...
import { linkHeaderImplementations } from './header-impl-linker.js';
import { linkGoInterfaces } from './go-interface-linker.js';
import { linkRustTraitImpls } from './rust-trait-linker.js';
import { linkCSharpPartials } from './csharp-partial-linker.js';
import { propagateLambdaCalls } from './lambda-call-propagator.js';
import { createResolutionContext } from './resolution-context.js';
import { seedSymbolTableFromCache, type SymbolCache } from './symbol-cache.js';
...
export interface PipelineOptions {
  ...
  /**
   * Restrict parsing to only these absolute file paths.
   * Structure (folders) is still built for all scanned files, but only
   * files in this set are parsed and resolved.  Used by --incremental.
   */
  fileFilter?: Set<string>;
  /**
   * Pre-populate the symbol table with exported symbols from a previous
   * build so files NOT in fileFilter can still participate in cross-file
   * resolution without being re-parsed.
   */
...
/**
 * Post-parse graph analysis: MRO and process extraction.
 *
 * Community detection is now handled post-DB-load via LadybugDB's native
 * Louvain algorithm (see processCommunitiesInDB in community-processor.ts).
 *
 * @reads  graph (all nodes and relationships from parse + resolve phases)
 * @writes graph (Process nodes, STEP_IN_PROCESS edges, OVERRIDES edges)
 */
async function runGraphAnalysisPhases(
  ...
  communityResult: CommunityDetectionResult | undefined;
  ...
  // ── Phase 5: Cross-file relationship linking ───────────────────────
  const headerImplEdges = linkHeaderImplementations(graph);
  if (isDev && headerImplEdges > 0) {
    console.log(`🔗 Header-impl linker: ${headerImplEdges} DECLARES/DEFINED_BY edges`);
  }

  const goImplEdges = linkGoInterfaces(graph);
  if (isDev && goImplEdges > 0) {
    console.log(`🔗 Go interface linker: ${goImplEdges} IMPLEMENTS edges`);
  }

  const rustImplEdges = linkRustTraitImpls(graph);
  if (isDev && rustImplEdges > 0) {
    console.log(`🔗 Rust trait linker: ${rustImplEdges} IMPLEMENTS edges`);
  }

  const csharpPartialEdges = linkCSharpPartials(graph);
  if (isDev && csharpPartialEdges > 0) {
    console.log(`🔗 C# partial linker: ${csharpPartialEdges} PARTIAL_OF edges`);
  }

  const lambdaPropEdges = propagateLambdaCalls(graph);
  if (isDev && lambdaPropEdges > 0) {
    console.log(`🔗 Lambda propagator: ${lambdaPropEdges} CALLS edges propagated`);
  }

  // Community detection deferred to post-DB-load (in-DB Louvain)
  ...
  const processResult = await processProcesses(
    graph,
    [], // Community memberships are populated post-DB-load via in-DB Louvain
    (message, progress) => {
  ...
  return { communityResult: undefined, processResult };
}
...
export const runPipelineFromRepo = async (
  ...
  try {
    // Seed symbol table from cache so cross-file resolution works for
    // files excluded by fileFilter (skipped during parsing).
    if (options?.symbolCache && options?.fileFilter) {
      const seeded = seedSymbolTableFromCache(ctx.symbols, options.symbolCache, options.fileFilter);
      if (isDev && seeded > 0) {
        console.log(`📦 Seeded ${seeded} cached symbols into resolution context`);
      }
    }
    ...
    // When fileFilter is set (incremental mode), only parse files in the filter
    //
    // Native parser integration (Phase 5A):
    //   The Rust native core (native-bridge.ts) supports parseFile/parseFiles for
    //   raw symbol extraction (12 languages, 63K files/sec). However, it ONLY
    //   extracts symbols (name, kind, startLine, endLine, isExported) — it does
    //   NOT handle imports, calls, heritage, routes, fetch calls, or tool defs.
    //   The TS parser extracts all of these in a single pass via tree-sitter.
    //   To wire Rust parsing, we would need to:
    //     1. Use Rust for symbol extraction on non-Swift files
    //     2. Keep TS parser for Swift (Rust Swift grammar is incomplete)
    //     3. Still run TS parser for import/call/heritage extraction on ALL files
    //   This makes the performance gain marginal since TS still processes every
    //   file for resolution. Deferred until Rust supports full extraction.
    //   CRITICAL: Do NOT wire Rust call resolution (3 tiers vs TS's 10 strategies).
    const filesToParse = options?.fileFilter
      ? scannedFiles.filter(f => options.fileFilter!.has(f.path))
      : scannedFiles;
    const { exportedTypeMap, allFetchCalls, allExtractedRoutes, allDecoratorRoutes, allToolDefs } = await runChunkedParseAndResolve(
      graph, ctx, filesToParse, allPaths, totalFiles, repoPath, pipelineStart, onProgress,
    );
    ...
    let communityResult: CommunityDetectionResult | undefined;
