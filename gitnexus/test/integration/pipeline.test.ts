...
describe('pipeline end-to-end', () => {
  ...
  it('community detection is deferred to post-DB-load (in-DB Louvain)', () => {
    // Community detection now runs inside LadybugDB after loadGraphToLbug,
    // not during the in-memory pipeline phase. The pipeline result should
    // NOT contain community data — that's populated by analyze.ts.
    expect(result.communityResult).toBeUndefined();

    // No Community nodes in the in-memory graph (they're created directly in DB)
    const communityNodes: string[] = [];
    result.graph.forEachNode(n => {
      if (n.label === 'Community') communityNodes.push(n.id);
    });
    expect(communityNodes.length).toBe(0);
  });
  ...
  it('reports progress through pipeline phases (communities deferred to DB)', () => {
    expect(phases).toContain('extracting');
    expect(phases).toContain('structure');
    expect(phases).toContain('parsing');
    // 'communities' phase no longer emitted — detection runs in-DB post-load
